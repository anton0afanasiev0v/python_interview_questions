...
<details>
<summary>Cтруктур данных в Python</summary>
...
Все три декоратора — это способы изменения поведения методов внутри класса, но они служат разным целям.
@property
Суть: Превращает метод в атрибут (в свойство).
Как использовать: Вы обращаетесь к нему как к атрибуту, без скобок ().
Для чего нужен:
Контроль доступа: Вы можете добавить логику при получении значения, его установке или удалении. Например, чтобы проверять корректность присваиваемого значения.
Вычисляемые атрибуты: Когда значение должно вычисляться динамически на основе других атрибутов объекта, но вы хотите, чтобы оно выглядело как обычное свойство.
Ключевая мысль: "Этот метод ведёт себя как данное, как поле объекта."

@staticmethod
Суть: Это функция, которая принадлежит пространству имён класса, но не имеет доступа ни к экземпляру (self), ни к самому классу (cls).
Как использовать: Вызывается из класса или экземпляра, но не получает автоматически первый аргумент.
Для чего нужен: Для логической группировки функций, которые имеют отношение к классу, но не требуют доступа к его данным. Это просто "удобное место" для хранения такой функции.
Ключевая мысль: "Эта функция имеет смысл в контексте этого класса, но она независима от любого конкретного объекта или самого класса."

@classmethod
Суть: Метод, который получает сам класс в качестве первого аргумента (по договорённости называется cls), а не экземпляр.
Как использовать: Всегда имеет доступ к классу, через который он вызван.
Для чего нужен:
Альтернативные конструкторы: Самый частый случай — создание экземпляров класса разными способами (например, из строки другого формата). Такие методы часто называют from_xxx().
Работа с состоянием класса: Если метод должен изменять или использовать атрибуты, общие для всех экземпляров (переменные класса).
Ключевая мысль: "Этот метод работает с классом в целом, а не с конкретным экземпляром."

Итог в одном предложении для каждого:
@property: "Ведёт себя как атрибут, но с логикой."
@staticmethod: "Обычная функция, но живёт внутри класса для удобства."
@classmethod: "Метод для работы с классом в целом, а не с его экземпляром."
...
```
Конечно! Вот полное описание для каждого исключения из иерархии:

**BaseException** - базовый класс для всех встроенных исключений.
├── SystemExit -> исключение, порождаемое функцией sys.exit() при выходе из программы.
├── KeyboardInterrupt -> порождается при прерывании программы пользователем (обычно сочетанием клавиш Ctrl+C).
├── GeneratorExit -> порождается при закрытии генератора или уничтожении объекта-генератора.
└── Exception -> базовый класс для всех стандартных исключений, которые не приводят к завершению программы.
    ├── StopIteration -> порождается для сигнализации о завершении итерации.
    ├── StopAsyncIteration -> порождается для сигнализации о завершении асинхронной итерации.
    ...
    ├── AssertionError -> порождается при неудачном выполнении оператора assert.
    ├── AttributeError -> возникает при неудачном обращении к атрибуту объекта (например, когда атрибут не существует).
    ├── BufferError -> возникает при невозможности выполнить операцию с буфером.
    ├── EOFError -> порождается функцией input() при неожиданном достижении конца файла (EOF).
    ├── ImportError -> возникает при неудачном импорте модуля или имени из модуля.
    │   └── ModuleNotFoundError -> возникает когда импортируемый модуль не может быть найден.
    ├── LookupError -> базовый класс для ошибок поиска.
    │   ├── IndexError -> возникает при обращении к несуществующему индексу последовательности (например, списка).
    │   └── KeyError -> возникает при обращении к несуществующему ключу словаря.
    ├── MemoryError -> возникает при нехватке памяти для выполнения операции.
    ├── NameError -> возникает при обращении к несуществующему имени (переменной).
    │   └── UnboundLocalError -> возникает при обращении к локальной переменной, которой еще не было присвоено значение.
    ├── OSError -> базовый класс для исключений, связанных с операционной системой.
    │   ├── BlockingIOError -> возникает когда операция блокируется на неблокирующем объекте.
    │   ├── ChildProcessError -> возникает при сбое в операции с дочерним процессом.
    │   ├── ConnectionError -> базовый класс для ошибок соединения.
    │   │   ├── BrokenPipeError -> возникает при записи в канал, у которого нет читающей стороны.
    │   │   ├── ConnectionAbortedError -> возникает когда соединение прервано peer'ом.
    │   │   ├── ConnectionRefusedError -> возникает когда соединение отклонено peer'ом.
    │   │   └── ConnectionResetError -> возникает когда соединение сброшено peer'ом.
    │   ├── FileExistsError -> возникает при попытке создать файл или директорию, которые уже существуют.
    │   ├── FileNotFoundError** -> возникает при попытке доступа к несуществующему файлу или директории.
    │   ├── InterruptedError** -> возникает когда системный вызов прерван входящим сигналом.
    │   ├── IsADirectoryError** -> возникает при попытке выполнить файловую операцию (например, открыть для записи) на директории.
    │   ├── NotADirectoryError** -> возникает когда операция ожидает директорию, но получает файл.
    │   ├── PermissionError** -> возникает при отсутствии прав доступа для выполнения операции.
    │   ├── ProcessLookupError** -> возникает когда процесс не найден.
    │   └── TimeoutError** -> возникает когда системная функция превышает timeout.
    ├── ReferenceError** -> возникает при обращении к слабой ссылке, когда объект уже не существует.
    ├── RuntimeError** -> возникает при ошибке, которая не попадает в другие категории.
    │   ├── NotImplementedError -> возникает когда абстрактный метод должен быть переопределен в подклассе.
    ...
    │   └── IndentationError -> базовый класс для ошибок отступов.
    │       └── TabError -> возникает при непоследовательном использовании табуляции и пробелов.
    ├── SystemError -> возникает при обнаружении внутренней ошибки в интерпретаторе Python.
    ├── TypeError -> возникает при выполнении операции или функции с объектом неподходящего типа.
    ├── ValueError -> возникает когда функция получает аргумент правильного типа, но неподходящего значения.
    │   └── UnicodeError -> базовый класс для ошибок, связанных с Unicode.
    │       ├── UnicodeDecodeError -> возникает при ошибке декодирования Unicode.
    │       ├── UnicodeEncodeError -> возникает при ошибке кодирования Unicode.
    │       └── UnicodeTranslateError -> возникает при ошибке трансляции Unicode.
    └── Warning -> базовый класс для категории предупреждений.
        ├── **DeprecationWarning** -> предупреждение об устаревших возможностях.
        ├── **PendingDeprecationWarning** -> предупреждение о возможном устаревании в будущем.
        ...
        ├── **FutureWarning** -> предупреждение об изменении семантики в будущих версиях.
        ├── **ImportWarning** -> предупреждение о возможных ошибках при импорте модулей.
        ├── **UnicodeWarning** -> предупреждения, связанные с Unicode.
        └── **BytesWarning** -> предупреждения, связанные с байтами и bytearray.
```
...
<details>
<summary>Вы проектируете систему. Как понять, когда нужно выбрать реляционную базу, а когда нереляционную?</summary>

Отличный вопрос! Это одно из ключевых архитектурных решений. Выбор между реляционными (SQL) и нереляционными (NoSQL) базами данных не сводится к тому, какая из них "лучше", а к тому, какая из них **правильный инструмент для ваших конкретных задач**.

Вот схема принятия решения, основанная на ключевых критериях.

### Краткая сводка-памятка

| Критерий | Реляционная (SQL) | Нереляционная (NoSQL) |
| :--- | :--- | :--- |
| **Данные** | Структурированные, строгая схема | Полу/неструктурированные, гибкая схема |
| **Масштабирование** | Вертикальное (более мощный сервер) | Горизонтальное (больше серверов) |
| **Целостность** | **ACID**-транзакции, ссылочная целостность | **BASE**-подход, eventual consistency |
| **Запросы** | Сложные JOIN, агрегации | Часто простые, по ключу или индексу |
| **Модель** | Таблицы, строки, столбцы | Документ, ключ-значение, граф, колоночная |

---

### Детальное объяснение

#### Выбирайте Реляционную Базу Данных (SQL), когда:

1.  **Данные структурированы и схема четко определена.**
    *   Ваши сущности (например, Пользователь, Заказ, Товар) имеют четкие атрибуты и связи между собой.
    *   Схема данных стабильна и не ожидается частых изменений.
    *   *Пример: Финансовая система (банки, бухгалтерия), системы бронирования.*

2.  **Крайне важна целостность данных (ACID).**
    *   **ACID** (Atomicity, Consistency, Isolation, Durability) — это золотой стандарт для транзакций.
    *   Вы не можете допустить ситуации, когда деньги списались с одного счета, но не зачислились на другой.
    *   *Пример: Любые операции с деньгами, складской учет, где важно точное количество.*

3.  **В ваших запросах много сложных JOIN и агрегаций.**
    *   Вам часто нужно объединять данные из множества таблиц для формирования отчета или сложной выборки.
    *   *Пример: "Покажи всех пользователей, которые купили товары из категории 'Электроника' в прошлом месяце, сгруппируй по городу и посчитай средний чек".*

4.  **Вы не планируете экстремального горизонтального масштабирования.**
    *   SQL-базы отлично масштабируются вертикально (увеличиваем CPU/RAM диску). Горизонтальное масштабирование (шардирование) возможно, но это сложная и нетривиальная задача.

#### Выбирайте Нереляционную Базу Данных (NoSQL), когда:

1.  **Данные полуструктурированные, неструктурированные или их схема быстро меняется.**
    *   Вы не можете четко определить все поля заранее. NoSQL позволяет гибко добавлять новые поля для разных записей.
    *   *Пример: Профили пользователей в соцсетях (у каждого разный набор интересов, связей), данные с IoT-датчиков, каталог товаров с разными атрибутами (для телефона — диагональ, для книги — автор).*

2.  **Требуется массивное горизонтальное масштабирование.**
    *   Вы ожидаете огромные объемы данных и высокую нагрузку на запись/чтение. NoSQL-базы изначально designed для легкого распределения данных across many servers.
    *   *Пример: Системы сбора логов, социальные сети, платформы для IoT.*

3.  **Модель данных и приложения ориентированы на высокую производительность операций "запись" и простых "чтений".**
    *   Часто данные читаются по первичному ключу или индексу, без сложных JOIN.
    *   *Пример: Кеширование (Redis), пользовательские сессии, корзина покупок в интернет-магазине.*

4.  **Допустима " eventual consistency" ( согласованность).**
    *   В распределенных системах не все узлы могут иметь актуальные данные в каждый момент времени, но они "догонят" и станут согласованными через короткое время. Это компромисс в пользу доступности и скорости.
    *   *Пример: Лайки и комментарии в соцсетях — не страшно, если счетчик лайков обновится через пару секунд.*

---

### Как подходить к выбору на практике? Вопросы, которые нужно задать себе:

1.  **Каков характер данных?** Они строго структурированы и связаны? Или это "куча" разнородных объектов?
2.  **Каковы требования к целостности?** Это банковский перевод или лента новостей?
3.  **Каковы ожидания по масштабированию?** Стартап с неизвестным трафиком или корпоративная CRM-система?
4.  **Какова частота и природа изменений схемы?** Мы каждый месяц добавляем новые поля к продукту?
5.  **Каков профиль запросов?** Много ли сложных отчетов или в основном простые выборки по ID?

### Частая гибридная архитектура ("Polyglot Persistence")

В современных сложных системах **часто используют оба типа баз одновременно**, каждая для своей задачи. Это называется "полиглотное хранение".

*   **Пример интернет-магазина:**
    *   **PostgreSQL (SQL):** для хранения основной информации о пользователях, заказах, товарах (транзакции, целостность).
    *   **Redis (NoSQL, ключ-значение):** для кеширования часто запрашиваемых товаров и корзин покупок (скорость).
    *   **Elasticsearch (NoSQL, поисковый движок):** для полнотекстового поиска по каталогу товаров.
    *   **MongoDB (NoSQL, документная):** для хранения логов действий пользователей на сайте (гибкость, объем).

### Итог

*   **Начинайте с реляционной БД**, если у вас нет явных и веских причин делать иначе. Это надежный, предсказуемый и хорошо изученный инструмент для 80% задач.
*   **Переходите на NoSQL**, когда вы уперлись в ограничения SQL по масштабированию, или когда природа ваших данных действительно не вписывается в реляционную модель.

Правильный выбор на ранних этапах сэкономит вам огромное количество времени и сил в будущем.
...
HAVING — это условие в языке SQL, которое используется для фильтрации результатов группировки, созданной оператором GROUP BY.
...
<details>
<summary>Основные преимущества асинхронности</summary>
